...
if __name__ == "__main__":
    ...
    for each in v_information:
        v_url = each['url']
        v_name = each['name']
        print('Downloading: ', v_name)
        # 这样每次只传一个url，还是单线程执行
        v_data = pool.map(glv.download_data, (v_url,))[0]  # pool.man()返回一个列表
        print('Completed: ', v_name)
        glv.save_data(v_name, v_data)
